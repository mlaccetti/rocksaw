...
/**
 * <p>
 * The RawSocket class provides a strictly utilitarian API for performing I/O
 * with IPv4 and IPv6 raw sockets. The API is minimalist, yet functional.
 * </p>
 * 
 * <p>
 * We throw java.io.InterruptedIOException when read/write operations time out
 * because java.net.SocketTimeoutException is present only in J2SE 1.4 and up.
 * By using InterruptedIOException, we allow programmers to use the software
 * with J2SE 1.2 and 1.3.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * <em>Important!  On most operating systems, you must have root
 * access or administrative privileges to use raw sockets.</em>
 * </p>
 */
public class RawSocket {
	...
	/**
	 * Initializes any system resources used by the RockSaw library. Really, all
	 * it does is call WSAStartup on Win32. It may be called multiple times (only
	 * the first call has any effect), but each call must be matched with a
	 * corresponding call to RockSawShutdown().
	 * 
	...
	/**
	 * Deallocates any system resources used by the RockSaw library. Really, all
	 * it does is call WSACleanup on Win32.
	 */
	...
	/**
	 * TimeVal is a convenience class for tracking select timeouts. Should we
	 * implement select outside of this class, we can make this a public top-level
	 * class with proper getters and setters.
	 */
	 ...
	 * starts with the string "SunOS", {@link #setUseSelectTimeout} is set to true
	 * (because Solaris does not support socket send and receive timeouts),
	...
	/**
	 * Writes a system error message into a StringBuffer. The message is appended
	 * to the supplied StringBuffer argument. This is a thread safe call on
	 * systems that support per-thread instances of errno.
	 * 
	 * @param buffer
	...
	/**
	 * <p>
	 * Returns the protocol number corresponding to the given protocol name. For
	 * example, {@code getProtocolByName("icmp");} should return 1 and
	 * {@code getProtocolByName("udp");} should return 17. The native system
	 * protocol database is used to look up the protocol numbers.
	 * </p>
	 * 
	 * <p>
	 * <em>This method really belongs in another class, probably in
	 * vserv-tcpip, but is currently included here as a convenience.  It
	 * may be moved elsewhere in the 1.0 release API.</em>
	 * </p>
	 * 
	 * @return The protocol number corresponding to the given protocol name. If
	 *         the protocol name cannot be found, returns a negative value.
	 */
	...
	/**
	 * <p>
	 * Returns by out parameter the address of the network interface that will be
	 * used to send a packet to the given destination. This works on Windows only
	 * and is necessary in order to compute ICMPv6 checksums.
	 * </p>
	 * 
	 * <p>
	 * <em>This method really belongs in another class,, but is
	 * currently included here for expediency.  It may be moved
	 * elsewhere in the 1.0 release API.</em>
	 * </p>
	 * 
	 * @param destination
	 *          The address of the destination.
	 * @param source
	 *          A byte array in which to store the returned source address. On
	 *          platforms other than Microsoft Windows, the array is left
	 *          unchanged.
	 * @exception IOException
	 *              If an I/O error occurs.
	 */
	public void getSourceAddressForDestination(InetAddress destination, byte[] source) throws IOException {
		if (__query_routing_interface(__socket, __family, destination.getAddress(), source) < 0) __throwIOException();
	}
	 ...
	 *          The protocol family of the socket (e.g., {@link #PF_INET} or
	 *          {@link #PF_INET6}).
	 * @param protocol
	 *          The protocol within the protocol family.
	 *          {@link #getProtocolByName} should be used to obtain protocol
	 *          numbers.
	 * @exception IllegalStateException
	 *              If the object instance is already open.
	 * @exception IOException
	 ...
	 * operating systems, binding a raw socket to an address causes only packets
	 * with a destination matching the address to be delivered to the socket.
	 * Also, the kernel will set the source address of outbound packets to the
	 * bound address (unless {@link #setIPHeaderInclude setIPHeaderInclude(true)}
	 * has been called).
	 * 
	 * @param address
	 *          The address to bind.
	...
	public void bind(InetAddress address) throws IllegalStateException, IOException {
		...
		if (__bind(__socket, __family, address.getAddress()) != 0) __throwIOException();
	}
	...
	/**
	 * Binds a network device (e.g., eth0) to a previously opened raw socket. This
	 * is implemented currently only for Linux using the SO_BINDTODEVICE socket
	 * option. Sent packets will leave through the bound device and only packets
	 * arriving via the device will be delivered to the socket.
	 * 
	 * @param device
	 *          The name of the device to bind (e.g., "eth0"). Passing a
	 *          zero-length string will remove the current binding. The loopback
	 *          interface ("lo") and device aliases (e.g., "eth0:1") cannot be
	 *          bound.
	 * @exception IllegalStateException
	 *              If the socket has not been opened first.
	 * @exception UnsupportedOperationException
	 *              If binding a device name is not supported on the runtime
	 *              platform.
	 * @exception IOException
	 *              If the device cannot be bound.
	 */
	public void bindDevice(String device) throws UnsupportedOperationException, IllegalStateException, IOException {
		...
		else if (result > 0) throw new UnsupportedOperationException();
	}
	...
	/**
	 * @return A negative value if an error occurs, else zero for false and a
	 *         positive value for true.
	 */
	private native static int __getIPHeaderInclude(int socket);

	/**
	 * Sets or unsets the IP_HDRINCL socket option. Setting this option causes
	 * IPv4 packet writes to expect the entire IP packet, starting from the
	 * header. The default behavior is to only expect the data payload. This
	 * option is valid only for IPv4 sockets.
	 * 
	 * @param on
	 *          True if headers should be included, false if not.
	 * @exception SocketException
	 *              If the option setting could not be altered.
	 */
	 ...
	 * @exception SocketException
	 *              If the option value could not be retrieved.
	 */
	 ...
	 * @exception SocketException
	 *              If the option value could not be set.
	 */
	...
	/**
	 * @return Zero if the socket is ready for I/O, negative value if timed out or
	 *         error occurred.
	 */
	...
	/**
	 * <p>
	 * Sets whether or not socket send/receive timeouts should be emulated by
	 * using the POSIX {@code select} function. Not all platforms support socket
	 * send/receive timeouts and this method provides a means to reproduce the
	 * same effect.
	 * </p>
	 * 
	 * <p>
	 * This method is not guaranteed to be retained in the 1.0 API. We may find a
	 * better way to provide support for read/write timeouts on all platforms.
	 * Technically, it's better to simply use non-blocking I/O rather than rely on
	 * socket timeouts, but we have yet to add a non-blocking I/O interface.
	 * </p>
	 * 
	 * @param useSelect
	 *          true if {@code select} should be used to implement timeouts, false
	 *          if not.
	 */
	...
	/**
	 * <p>
	 * Determines whether or not socket send/receive timeouts are emulated by
	 * using the POSIX {@code select} system function. Not all platforms support
	 * socket send/receive timeouts. The default value is false except for
	 * platforms where the {@code os.name} property starts with the string
	 * "SunOS".
	 * </p>
	 * 
	 * @return True if send/receive timeouts are emulated with select, false if
	 *         not.
	 */
	...
	/**
	 * Sets the send timeout (SO_SNDTIMEO). A timeout of zero indicates an
	 * infinite timeout. A negative timeout is undefined.
	 * 
	 * @param timeout
	...
	/**
	 * Sets the receive timeout (SO_RCVTIMEO). A timeout of zero indicates an
	 * infinite timeout. A negative timeout is undefined.
	 * 
	 * @param timeout
	 ...
	 * delivered in their entirety, including the IP header. IPv6 (
	 * {@link #PF_INET6}) packet data will not include the IPV6 header.
	 * 
	 * @param data
	 *          The buffer in which to store the packet data.
	 * @param offset
	 *          The offset into the buffer where the data should be stored.
	 * @param length
	 *          The number of bytes to read.
	 * @param address
	 *          A byte array in which to store the source address of the received
	 *          packet. It may be null if you don't want to retrieve the source
	 *          address. Otherwise, it must be the right size to store the address
	 *          (e.g., 4 bytes for an IPv4 address).
	 * @exception IllegalArgumentException
	 *              If the offset or lengths are invalid or if the address
	 *              parameter is the wrong length.
	 * @exception IOException
	 *              If an I/O error occurs.
	...
	public int read(byte[] data, int offset, int length, byte[] address) throws IllegalArgumentException, IOException, InterruptedIOException {
		...
		if (address != null && ((__family == PF_INET && address.length != 4) || (__family == PF_INET6 && address.length != 16))) throw new IllegalArgumentException("Invalid address length.");

		int result = 0;

		if (getUseSelectTimeout() && !__rtimeout.isZero()) result = __select(__socket, true, __rtimeout.seconds, __rtimeout.microseconds);

		if (result == 0) result = (address == null ? __recvfrom1(__socket, data, offset, length, __family) : __recvfrom2(__socket, data, offset, length, __family, address));
	...
	/**
	 * Writes packet data to the socket. The data should not include the IP
	 * header. IPv4 ({@link #PF_INET}) sockets may set the IP_HDRINCL option with
	 * {@link #setIPHeaderInclude}, in which case the packet data should include
	 * the IP header.
	 * 
	 * @param address
	 *          The destination to write to.
	 * @param data
	 *          The buffer from which to copy the packet data.
	 * @param offset
